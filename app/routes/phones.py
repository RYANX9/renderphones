...
from ..core.database import get_pool, acquire_conn  # add acquire_conn
...
from ..core.market import resolve_offers_for_region
...
async def recommend_phones(
    ...
    region: Optional[str] = Query(None, description="ISO-3166 alpha-2, e.g. 'US', 'DE'"),
):
    ...
    async with acquire_conn() as conn:
        result = await recommend_svc(
            conn, priorities=priority_list, min_price=min_price, max_price=max_price,
...
@router.get("/{phone_id}")
async def get_phone(phone_id: str, region: Optional[str] = Query(None)):
    async with get_pool().acquire() as conn:
        ...
        retailer_links = await phone_repo.fetch_retailer_links(conn, phone["id"])
    ...
    phone["retailer_offers"] = resolve_offers_for_region(retailer_links, region)
    return phone
...
async def similar_phones_route(
    phone_id: int,
    ...
    async with get_pool().acquire() as conn:
        ...
        if not exists:
            raise HTTPException(status_code=404, detail=f"Phone {phone_id} not found.")
        phones = await phone_repo.similar_phones(conn, phone_id, limit, region=region)
...
@router.get("/{phone_id}/offers")
async def get_phone_offers(phone_id: int, region: Optional[str] = Query(None)):
    async with get_pool().acquire() as conn:
        exists = await conn.fetchval("SELECT 1 FROM phones WHERE id = $1", phone_id)
        if not exists:
            raise HTTPException(status_code=404, detail=f"Phone {phone_id} not found.")
        rows = await phone_repo.fetch_retailer_links(conn, phone_id)
    return resolve_offers_for_region(rows, region)