...
from ..core.market import disallowed_retailers_for_region, cap_per_brand
...
CATEGORY_BRAND_CAP = 2
# Overfetch factor so capping + region exclusion still leaves `limit`
# results without a second round trip.
CATEGORY_OVERFETCH_MULTIPLIER = 3
...
async def get_category(
    ...
    async def _fetch():
        ...
        params: list = []
        if region:
            blocked = disallowed_retailers_for_region(region)
            if blocked:
                region_clause = (
                    "AND (NOT EXISTS (SELECT 1 FROM retailer_links rl WHERE rl.phone_id = p.id) "
                    "OR EXISTS (SELECT 1 FROM retailer_links rl2 WHERE rl2.phone_id = p.id "
                    "AND LOWER(rl2.retailer) != ALL($1::text[])))"
                )
                params.append(blocked)

        fetch_limit = limit * CATEGORY_OVERFETCH_MULTIPLIER
        sql = cfg["sql"].format(cols=_CATEGORY_COLS, limit=fetch_limit, region_clause=region_clause)
        ...
        phones = cap_per_brand(phones, CATEGORY_BRAND_CAP)[:limit]

        return {"slug": resolved, "title": cfg["title"], "description": cfg["description"], "phones": phones}

    cache_key = f"category:{resolved}:{limit}:{(region or 'none').upper()}"
    return await cached(cache_key, settings.cache_ttl_stable, _fetch)