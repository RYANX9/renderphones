...
from .market import disallowed_retailers_for_region
...
class FilterParams:
    ...
    # ISO-3166 alpha-2 (e.g. "US", "DE"). When set and in BLOCKED_REGIONS,
    # phones whose entire retailer_links coverage is china-domestic-only
    # retailers are excluded. Phones with zero retailer_links rows are
    # never excluded — absence of data means "unscoped", not "blocked".
...
def build_filter_where(f: FilterParams) -> tuple[str, list[Any], str | None]:
    ...
    if f.region:
        disallowed = disallowed_retailers_for_region(f.region)
        if disallowed:
            conditions.append(
                f"(NOT EXISTS (SELECT 1 FROM retailer_links rl WHERE rl.phone_id = p.id) "
                f"OR EXISTS (SELECT 1 FROM retailer_links rl2 WHERE rl2.phone_id = p.id "
                f"AND LOWER(rl2.retailer) != ALL(${i}::text[])))"
            )
            params.append(disallowed)
            i += 1
