...
# The one place "what price do we show" gets decided. LEAST() in Postgres
# ignores NULL arguments (only returns NULL if every argument is NULL), so
# this is genuinely "minimum of whichever of these three exist" — variant
# floor, latest tracked price_point (global scope preferred, matching
...
{PRICE_RESOLVED_EXPR} AS price_usd, p.release_year, p.availability_status, p.popularity,
...
{PRICE_RESOLVED_EXPR} AS price_usd, p.price_original, p.currency, p.price_updated_at,
...
{PRICE_RESOLVED_EXPR} AS price_usd, p.price_original, p.currency, p.availability_status,
...
SORT_COL_MAP: dict[str, str] = {
    ...
    "price_usd": "price_usd",  # the resolved LEAST(...) alias from the SELECT list, not p.price_usd
